...
int check_ports() {
    ...
    while (fgets(buffer, sizeof(buffer), fp)) {
        if (strstr(buffer, "Netid") || strstr(buffer, "State")) continue;
        ...
        if (strstr(state, "LISTEN") == NULL) continue;
        ...
        if (is_dangerous(port)) {
            ...
            switch(port) {
                case 22: printf("SSH brute-force attacks"); break;
                case 23: printf("Cleartext credentials"); break;
                case 80: case 443: printf("Web server vulnerabilities"); break;
                case 139: case 445: printf("EternalBlue exploits"); break;
                case 3389: printf("RDP brute-forcing"); break;
                case 4444: printf("Metasploit payloads"); break;
                case 5555: printf("Android backdoors"); break;
                case 31337: printf("Classic backdoor port"); break;
                default: printf("Known backdoor/exploitable service");
            }
            printf("\n-------------------------------------\n");
            found_danger = 1;
        }
